<?php if ( get_theme_mod( 'featured-content' ) == "Display" ) { ?>
	<div class="productshowcase">
		<?php $customcolors = get_theme_mod( 'featured-colors' );
		if ($customcolors == "on") {
		...
		<div class="widgetwrap">
			<div class="titlewrap610"><h2><?php echo get_theme_mod( 'featured-title' ); ?></h2></div>
			<div class="widgetbody">
				...
				<div class="homefeatured">
					...
					<?php $category = get_theme_mod( 'featured-cat-1' );
					$count          = 0;
					...
					if ( have_posts() ) : while ( have_posts() ) : the_post();
						...
						if ( ( $count == 1 ) && ( get_theme_mod( 'featured-hide-cat-1' ) == "No" ) ) { ?><a
							href="<?php echo cat_id_to_slug( $category ); ?>"><p class="viewall">CAMPING GEAR
									REVIEWS</p></a><?php } ?>
						...
						<?php if ( has_post_thumbnail() ) {
							...
							if ( $attachment ) { ?><a href="<?php the_permalink(); ?>"><img
									src="<?php echo $attachment; ?>" class="featureimage" style="width:180px"/>
								</a><?php }
						} ?>

						<a href="<?php the_permalink() ?>" rel="bookmark"
						   title="Permanent Link to <?php the_title(); ?>"><h3 class="homeheadline"
						                                                       style="font-size:14px;line-height:22px;"><?php the_title(); ?></h3>
						</a>
						<?php $teaser = get_theme_mod( 'featured-teaser-1' );
						if ( $teaser ) {
							the_content_limit( $teaser, "<br /><span class='readmore'>Read More</span>" );
						} ?>
				...
				<div class="homefeatured">
					...
					<?php $category = get_theme_mod( 'featured-cat-2' );
					$count          = 0;
					query_posts( 'cat=' . $category . '&showposts=1' );
					if ( have_posts() ) : while ( have_posts() ) : the_post();
						global $post;
						...
						if ( ( $count == 1 ) && ( get_theme_mod( 'featured-hide-cat-2' ) == "No" ) ) { ?><a
							href="<?php echo cat_id_to_slug( $category ); ?>"><p
								class="viewall"><?php echo cat_id_to_name( $category ); ?></p></a><?php } ?>
				...
				<div class="homefeaturedright">
					...
					<?php $category = get_theme_mod( 'featured-cat-3' );
					$count          = 0;
					query_posts( 'cat=' . $category . '&showposts=1' );
					if ( have_posts() ) : while ( have_posts() ) : the_post();
						global $post;
						...
						if ( ( $count == 1 ) && ( get_theme_mod( 'featured-hide-cat-3' ) == "No" ) ) { ?><a
							href="<?php echo cat_id_to_slug( $category ); ?>"><p
								class="viewall"><?php echo cat_id_to_name( $category ); ?></p></a><?php } ?>
