...
* Added ability to change location of filter menu
* Added ability to replace standard filter menus with Select2 widget
* Added ability to filter events by author
* Fixed timezone issues on front-end event creation form
* Fixed issue with Bootstrap modal conflicts
* Added support for multiple category/tag selection on calendar feed
file upload form
* All JavaScript and CSS is served through PHP to make
browser-sensitive adjustments
* Upgraded jQuery to 2.0 where supported
* Latitude and longitude are stored when available
* Upgrade notice is now displayed to network administrators
* Fixed conflict with JetPack plugin
* Modified exported UID to reflect event origin
* Fixed issue where some names got "MAILTO:" prefix when importing feeds
* Fixed visual issues with posterboard tiles
* Added better support for free events
* Fixed memory leak which was causing admin area to be rendered
unusable
* Adjusted view cache for better performance and more reliable results
* Added "last" option to monthly recurrence patterns
* Fixed issue where Facebook updates were failing under certain
conditions
* Changed logic between categories and tags filters from OR to AND

= Version 1.10.1-pro =
* Fixed issue where times were displayed in incorrect timezone
* Fixed issue that caused "Add New Event" page to remain blank on some
servers
...
* Improved event start/end time display in Posterboard and Stream
views; "Show year" setting now also applies to Posterboard view
* Added e-mail notifications for site admins when new events are
submitted via front-end form
* Added ability for users to be notified by e-mail for individual
events 6 hours before they start
* Added ability to publish events to Facebook Pages
